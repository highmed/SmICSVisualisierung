...
class Linelist extends Component {
  constructor(props) {
    ...
    this.title = "new annotation timeline"
  ...
  update_tooltip = (d, i) => {
    ...
    if (i === "symptomRect") {
      tableObj = {
        title: "SYMPTOM - Patient " + d.patientID,
        header: ["DataName", "Value"],
        content: [
          ["SymptomArt", d.symptomArt],
          [
            "Symptom-Beginn",
          ...
          [
            "Symptom-Ende",
            moment(d.symptomEnde).format(this.translate("dateFormat")) + " Uhr",
          ],
        ],
      }
    } else if (i === "virusLastRect") {
      tableObj = {
        title: "VIRUSLAST - Patient " + d.PatientID,
        header: ["DataName", "Value"],
        content: [
          [
            "Befunddatum",
            moment(d.Befunddatum).format(this.translate("dateFormat")) + " Uhr",
          ],
          [
            "Zeitpunkt Probeneingang",
            moment(d.ZeitpunktProbeneingang).format(
              this.translate("dateFormat")
            ) + " Uhr",
          ],
          ["Befundkommentar", d.Befundkommentar],
          ["Material", d.Material_l],
          // ["Viruslast", d.Viruslast],
          ["quantitatives Ergebnis (ct-Wert)", d.Viruslast],
        ],
      }
    } else if (i === "spritze") {
      tableObj = {
        title: "IMPFUNG - Patient " + d.PatientenID,
        header: ["DataName", "Value"],
        content: [
          [
            "Doku-Zeitpunkt",
          ...
          ["Dosiermenge", d.Dosiermenge],
          ["Impfstoff", d.Impfstoff],
          ["Impfungen gegen", d.ImpfungGegen],
        ],
      }
    } else if (i === "link") {
  ...
  virusLastColorDefiner = (d) => {
    ...
    if (d.Viruslast < 15) return "rgb(252,141,89)"
    if (d.Viruslast < 30) return "rgb(255,255,191)"
    return "rgb(145,207,96)"
  }

  virusLastOpacityDefiner = (d) => {
    let o = 1
    let last = d.Viruslast
    if (last < 15) {
      last = -last
      last += 15
