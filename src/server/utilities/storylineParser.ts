const parseToStorylineData = (input: any, specificKeimID: any) => {
   ...
   */
  let lop = parameters.lop[0]
  let stationList: any[] = []
  let { microData, movementData } = rawData
  ...
  microData.forEach((untersuchung: any) => {
  ...
  movementData.forEach((bewegung: any) => {
  ...
  lop.forEach((patientID: any) => {
     ...
     */
    Object.getOwnPropertyNames(keimeInfos).forEach((keimID: any) => {
      ...
      if (keimObject.positive.length > 0) {
        ...
        negative = keimObject.negative.filter(
          (ts: any) => ts < keimObject.erstesPositiv
        )
      } else {
  ...
  keime.lok.forEach((keimID: any) => {
    keime["K" + keimID].erstePositiveBefunde.forEach((befund: any) => {
    ...
    keime["K" + keimID].letzteNgativeBefunde.forEach((befund: any) => {
      let index = allRelevantInspections.findIndex(
        (ri) =>
          ri.timestamp === befund.timestamp && ri.patientID === befund.patientID
      )
  ...
  let eventsCategorized: any = {}
  let graphsCategorized: any = {}
  let nodesByTimestamp: any = {}
  keime.lok.forEach((keimID: any, keimCounter: any) => {
    if (keimID !== Number(specificKeimID)) {
      ...
      let e: any = (nodesByTimestamp["K" + keimID] = [])
